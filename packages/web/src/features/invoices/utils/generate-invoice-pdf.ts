...
export function generateInvoicePdf(params: GenerateInvoicePdfParams) {
  ...
  doc.setFontSize(9);
  doc.text(invoiceNumber, pageWidth - margin, headerH / 2 + 6, { align: "right" });

  let y = headerH + 10;

  // ── SENDER INFO (left) + META INFO (right) — two independent columns ──
  //
  // Left column: sender contact details
  ...
  const vatNumber = formatVatNumber(profile.orgNumber);
  ...
  // --- Right: invoice meta ---
  const metaItems = [
    { label: l.invoiceNumber, value: invoiceNumber },
    { label: l.date, value: invoiceDate },
    { label: l.dueDate, value: dueDate },
    ...(invoiceRef ? [{ label: l.invoiceRef, value: invoiceRef }] : []),
    { label: l.overdueInterest, value: `${overdueInterestRate}%` },
  ];
  ...
  const colX = {
    desc: margin,
    qty: margin + contentWidth * 0.46,
    price: margin + contentWidth * 0.60,
    vat: margin + contentWidth * 0.74,
  ...
  doc.setFontSize(8);
  doc.setTextColor(brandR, brandG, brandB);
  doc.text(l.payment.toUpperCase(), margin, y);
  y += 5;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8.5);
  doc.setTextColor(...primary);
